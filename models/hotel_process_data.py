...
def load_data():
    train = _parse_data(open('data/hotel_train_data.data', 'r'))
    test = _parse_data(open('data/hotel_test_data.data', 'r'))
    ...
    with open('saved_model/hotel_zh_config.pkl', 'wb') as outp:
...
def _process_data(data, vocab, chunk_tags, maxlen=None, onehot=False):
    ...
    # x = pad_sequences(x, maxlen)  # right padding
...
def process_data(data, vocab, maxlen=None):
    if maxlen is None:
        maxlen = len(data)
    word2idx = dict((w, i) for i, w in enumerate(vocab))
